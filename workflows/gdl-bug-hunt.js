...
THE CORPUS - this is the point of the exercise:
  ...
  - "${SEEDS}"/*.gdl                  14 themed seed projects (Git LFS)
...
// batch against all 19 projects. Pipelined - no barrier, a batch starts being
// falsified the moment its extractor returns.
// ---------------------------------------------------------------------------
const CLAIM_SOURCES = [
  {
    key: 'docs',
...
// ---------------------------------------------------------------------------
// Phase 3: eight independent bug-class sweeps. Each is blind to the others.
// ---------------------------------------------------------------------------
const DIMENSIONS = [
  {
...
severity - overstating an impact is its own kind of wrong.`,
    ...
    .then(votes => {
      ...
      // Unanimous survival required: two lenses, either one can kill it.
