...
| `gdl/behavior.py` | what controls do: checks, and the Global Scripter program plus handoff generated from it |
...
| `tests/verify_behavior.py` | checks a generated program's page names and control IDs against the built panel |
...
python -m gdl.behavior check    examples/huddle-program.json   # what the controls do
...
A spec can also say what its controls do; `gdl.behavior` checks that and writes
the Global Scripter program and a programmer's handoff from the same file
(`docs/behavior.md`).
