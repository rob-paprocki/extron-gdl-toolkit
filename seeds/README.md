...
Fifteen themed projects made in GUI Designer with **File > New Project**, used
...
is how `Afterburn ECP 16-9 (Project1).gdl` was made. The one thing to avoid is
setting a combo's value as text: that updates the edit field without raising the
selection, so the wizard reads correct and stays internally unselected, with
Create disabled and no way back but Cancel. `docs/from-scratch.md` §5c has the
recipe and §7 the automation detail.
...
| `Afterburn ECP 16-9 (Project1).gdl` | Afterburn All-inclusive 1720 | VTLPEcp | 60-sVTLPEcp | 1920x1080 | 7 | 29 | 649 | 31 | Arial, Extron-Afterburn, Open Sans |
...
- **Fonts differ too.** The Zoom Rooms seeds declare only Arial and Arial Black;
  ...
  face is not always an embedded one — see `gdl/fonts/README.md`. The ECP seed is
  the one Afterburn project that does **not** declare Arial Black, so a spec that
  names it resolves in every other Afterburn seed and not that one.
- **The ECP seed is a soft client, and its canvas is a choice.** Every other seed
  is locked to its panel's resolution. Extron Control Pro offers seven
  (`docs/design-rules.md` §1); this seed is the TouchLink Panel (16:9) preset, so
  it is 1920 × 1080 and the project behind it is Extron's 1720 template.
...
ECP is covered at 16:9 only. Its other six presets — the phone and tablet sizes,
and TouchLink Panel (16:10) — have no seed, and Mach and Shockwave have no ECP
seed at all.
