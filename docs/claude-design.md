...
| **Button archetypes** | The looks the seed's own buttons use, per state. Afterburn has three: outlined (the most common), ghost and inverse. |
...
| Slider, Level | `slider`, `level` | `orientation`, `fill`, `does` |
...
- Each artboard beside the built panel's render for sign-off, since Claude
  Design does not draw like GUI Designer and `gdl.spec render` is measured
  against it (`CLAUDE.md` *Verifying a change*).
